...
int pasar (nodoAuto *lista, automovil A[], int dim)
{
    ...
    if (lista!=NULL)
    {
        insertarCelda(A, dim, lista->dato, validos);
    }
...
void mostrarArregloRecursivo(automovil A[ ], int i, int pos)
{
    if (i>pos)
    {
        ...
        printf ("Patente: %s", A[pos].patente);
        printf ("Marca: %s", A[pos].marca);
...
int sumaRecursivaPar (automovil A[], int i, int pos)
{
    ...
    if(i>pos)
    {
        flag=determinaSiEsPar(A[pos].patente);

        if (flag==1)
        {
            suma=suma+A[pos].valor;
            suma=sumaRecursivaPar(A, i, pos+1);
        }
        else
        {
            suma=sumaRecursivaPar(A, i, pos+1);
